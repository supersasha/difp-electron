...
float sigma_constr(float x, float xmin, float xmax, float ymin, float ymax, float smoo)
{
    // gamma = (ymax - ymin) / (xmax - xmin)
    float gamma = (ymax - ymin) / (xmax - xmin);
    return sigma_to(x, ymin, ymax, gamma, smoo, xmax);
}
...
#define T(i, j) (u_spectrumData.tri_to_v_mtx[i].a[j])
    ...
    for (int i = 0; i < SPECTRUM_SIZE; i++) {
        float developed_dyes = u_profileData.film_dyes[0].a[i] * dev.x
                             ...
                             ;
        float developed_couplers =
            u_profileData.couplers[0].a[i] * (1.0 - dev.x / u_profileData.film_max_qs[0])
          + u_profileData.couplers[1].a[i] * (1.0 - dev.y / u_profileData.film_max_qs[1])
          + u_profileData.couplers[2].a[i] * (1.0 - dev.z / u_profileData.film_max_qs[2]);
    ...
    float L = 0.0;
    if (u_userOptions.mode != NEGATIVE) {
        H = log10(exposure) + u_userOptions.paper_exposure - mask;
        ...
        // Viewing paper
        for (int i = 0; i < SPECTRUM_SIZE; i++) {
            float r = sigma_to(H.x, 0.0, 4.0, u_profileData.paper_gammas[0] * u_userOptions.paper_contrast, u_userOptions.curve_smoo, 0.0);
            float g = sigma_to(H.y, 0.0, 4.0, u_profileData.paper_gammas[1] * u_userOptions.paper_contrast, u_userOptions.curve_smoo, 0.0);
            float b = sigma_to(H.z, 0.0, 4.0, u_profileData.paper_gammas[2] * u_userOptions.paper_contrast, u_userOptions.curve_smoo, 0.0);
